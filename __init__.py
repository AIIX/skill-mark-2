...
class Mark2(MycroftSkill):
    ...
    def on_register_idle(self, message):
        if 'name' in message.data and 'id' in message.data:
            self.idle_screen_list.append({"screenName": message.data['name'], "screenID": message.data['id']})
            self.idle_screens["screenBlob"] = self.idle_screen_list
